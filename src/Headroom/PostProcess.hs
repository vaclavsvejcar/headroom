...
postProcess :: PostProcess env -- ^ /post-processor/ to run
...
-- given configuration.
postProcessHeader :: ConfiguredEnv -- ^ configuration used to define post-processing behaviour
...
data ConfiguredEnv = ConfiguredEnv
  { ceCurrentYear         :: CurrentYear          -- ^ current year
  , cePostProcessConfigs  :: CtPostProcessConfigs -- ^ configuration of /post-processor/
  , ceUpdateCopyrightMode :: UpdateCopyrightMode  -- ^ mode used by the 'updateCopyright' /post-processor/
  }
  deriving (Eq, Show)
